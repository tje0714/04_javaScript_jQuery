...
*/
  ...
  // userList 로컬스토리지에 유저정보들을 보유하고
  // 추후 새로운 데이터를 저장할 변수이름

  userList.push(newUser); // 기존 userList 에 새로운 유저 추가한 상태!
  // 로컬스토리지가 아닌 javaScript 내에서만 추가하겠다 되어있는 상태
...
function searchData(e) {
  ...
  .filter() : 걸러낼 것이다.
  data      : data 라는 변수 이름에 userList에서 가져온 정보를 하나씩 담아서
  ...
  */
  ...
  if (searchResult.length > 0) {
    // 검색 결과를 보여줄 것
    /*
    filter 내부에 작성된 data는 filter 에서 소비자가 검색한 데이터와
    일치하는지 확인하기 위해 사용
    
    map 내부에 작성된 data 는 검색완료된 유저 목록을
...
function clearAllData(e) {
  ...
  if (confirm("정말로 모든 사용자를 삭제하시겠습니까?")) {
    ...
    // localStorage.clear();                = 로컬스토리지 내부에 존재하는 모~~~든 데이터 삭제
    localStorage.clear();
