...
// 해당하는 URL주소를 클릭하여 내부에 저장된 키 값 확인
// 키 값은 문자열로 무조건 되어있음
// "" 가 없다하여 문자열이 아닌 것이 아니라 생략상태!!!

// 인터넷 창에서 크롬일경우 chrome://version 입력
// 프로필 경로	C:\Users\tj\AppData\Local\Google\Chrome\User Data\Profile 5
// 프로필 경로를 따라서 폴더 내부를 들어가면 local Storage 폴더가 존재할 것
// 폴더 내부에 db 나 dbl로 저장된 내부에 컴퓨터가 읽을 수 있는 글씨로 존재
...
function saveDataFn(e) {
  ...
  // a 태그 속성 기본값 href 로 들어가는 것을 방지
  // (예 : 마이페이지, 임직원페이지 접근 가능한 유저인가?)
  e.preventDefault(); // 제출 방지기능  type = "button" 일 경우 사용 안해도 됨

  //소비자가 작성한 키와 값으로 저장할 예정
  const inputKey = $("#key").val(); // 기본 자바스크립트에서는 value 로 사용
  const inputValue = $("#value").val(); // 기본 자바스크립트에서는 value 로 사용

  // 실제 localStorage에 저장!
  // .setItem() : 기능 내부에는 키와 값을 저장한다 -> 크롬에서 기본적으로 제공하는 저장할 위치에
  // 소비자가 작성한 데이터의 키와 값을 저장해놓는 경로 까지 작성
...
// 키의 이름을 가져올 때는 index 번호를 활용해서 0번 째에 존재하는 key 명칭을 가져온다.
...
// for 문 보다 로컬스토리지에 리스트목록을 저장하는 것이 메모리 활용적
